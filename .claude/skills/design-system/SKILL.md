...
This skill delegates to specialist agents for domain expertise. The master session
...
- The agent returns analysis/proposals to the master session
- The master session presents the agent's output to the user via `AskUserQuestion`
- The user decides; the master session writes to file
- Agents do NOT write to files directly — the master session owns all file writes
